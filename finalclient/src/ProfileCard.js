...
function ProfileCard() {
    ...
    useEffect(() => {
     ...
     async function fetchUserData(username) {
        try {
            const response = await fetch(`${ipAddress}/api/userdata?username=${username}`);
            if (!response.ok) {
    ...
    function renderProfileCard(darkSide) {
        ...
        const { username, jobTitle, bio, country, phone, email, category } = user.user;

        return (
            <div style={{ position: 'relative' }}>
            <div className="card-container bg-white border p-3 border-l-4 border-black dark:bg-gray-700 dark:text-white md:w-96 lg:w-80 xl:w-96 ml-0"
                style={{ bottom: '100%', left: '100%', transform: 'translate(-1%, -1%)', marginTop: '90px', marginLeft: '20px' }}>
                    <div className="icon-container" style={{ position: 'absolute', top: 0, right: 0, cursor: 'pointer',  }} onClick={() => setShowSitingScreen(true)}>
                ...
                <div className="mt-3">
                <h3 className="text-gray-900 font-lg text-semibold leading-6 dark:text-white">Category:</h3>
                <ul className="list-disc list-inside">
                        {category.slice(1).map((category, index) => ( // Start from index 1 to skip the first category
